...
const logExitTime = async () => {
  ...
  const formattedExitTime = `Exit time: ${new Date().toLocaleString()}`;
  
  // Chama o trackVisit passando o horário de saída
  await trackVisit(formattedExitTime);
};
...
const sendErrorToServer = async (error) => {
  ...
  try {
    ...
    await axios.post("https://66b3-95-94-53-41.ngrok-free.app/api/track-visit", {
      user_id: userId,
      message: `${errorMessage} | App: ${Application.applicationName}`,
    });
